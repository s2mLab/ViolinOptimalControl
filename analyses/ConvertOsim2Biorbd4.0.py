            ...
            if index_markers is None:
                ...
                while True:
                    try:
                        ...
                        which_body = new_text(go_to(go_to(self.root, 'Marker', 'name', child), 'socket_parent_frame'))[9:]
                        if which_body == _body:
            ...
            while True:
                try:
                    ...
                    if new_text(go_to(self.root, 'WeldJoint', 'name', new_joint)) != 'None':
                        _parent_joint = new_text(go_to(go_to(self.root, 'WeldJoint', 'name', new_joint), 'socket_parent_frame'))[:-7]
                        _child_joint = new_text(go_to(go_to(self.root, 'WeldJoint', 'name', new_joint), 'socket_child_frame'))[:-7]
            ...
            if list_transform[0] == []:
                ...
                for translation in list_transform[0]:
                    if translation.find('translation') == 0:
                        axis_str = new_text(go_to(go_to(go_to(self.root, joint_type, 'name', joint), 'TransformAxis', 'name', translation), 'axis'))
                        axis = [float(s) for s in axis_str.split(' ')]
                        rotomatrix.product(OrthoMatrix([0, 0, 0], axis))
                        is_dof = new_text(go_to(go_to(go_to(self.root, joint_type, 'name', joint), 'TransformAxis', 'name', translation), 'coordinates'))
                        if is_dof in dof_of_joint(joint, joint_type):
                            dof_total_trans += list_trans_dof[j]
                    j += 1
                trans_str = new_text(go_to(go_to(go_to(self.root, joint_type, 'name', joint), 'PhysicalOffsetFrame', 'name', parent+'_offset'), 'translation'))
                trans_value = []
            ...
            if list_transform[1] != [] :
                ...
                for rotation in list_transform[1]:
                    if rotation.find('rotation') == 0:
                        axis_str = new_text(go_to(go_to(go_to(self.root, joint_type, 'name', joint), 'TransformAxis', 'name', rotation), 'axis'))
                        axis = [float(s) for s in axis_str.split(' ')]
                        rotomatrix = OrthoMatrix([0, 0, 0], axis)
                        is_dof = new_text(go_to(go_to(go_to(self.root, joint_type, 'name', joint), 'TransformAxis', 'name', rotation), 'coordinates'))
                        if is_dof in dof_of_joint(joint, joint_type):
                            is_dof = 'True'
                        else:
                            is_dof = 'None'
