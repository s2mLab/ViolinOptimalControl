...
class BiorbdModel:
        ...
        while number_line < len(self.words):
            ...
            if line[0] == "endmusclegroup" and is_muscle_group:
                new_muscle_group = MuscleGroup(
                    muscle_group_name, muscle_group_origin_parent, muscle_group_insertion_parent
                )
            ...
            if line[0] == "endviapoint" and is_viapoint:
                new_pathpoint = Pathpoint(
                    pathpoint_name, pathpoint_parent, pathpoint_muscle, pathpoint_muscle_group, pathpoint_position
                )
    ...
    def write_marker(self, segment_index, marker_index):
        ...
        self.file.write(
            "\n\t\tposition\t{}\t{}\t{}".format(
                marker.get_position()[0], marker.get_position()[1], marker.get_position()[2]
            )
        )
        self.file.write("\n\tendmarker\n")
        return 0
    ...
    def write_muscle(self, muscle_group_index, muscle_index):
        ...
        self.file.write(
            "\n\t\tOriginPosition\t{}\t{}\t{}".format(start_pos[0], start_pos[1], start_pos[2])
        ) if start_pos else self.file.write("")
        self.file.write(
            "\n\t\tInsertionPosition\t{}\t{}\t{}".format(insert_pos[0], insert_pos[1], insert_pos[2])
        ) if insert_pos else self.file.write("")
        ...
        ) if tendon_slack_length != "" else self.file.write("")
        self.file.write(
            "\n\t\tpennationAngle\t{}".format(pennation_angle)
        ) if pennation_angle != "" else self.file.write("")
    ...
    def write_pathpoint(self, muscle_group_index, muscle_index, pathpoint_index):
        ...
        self.file.write("\n\t\t\tmusclegroup\t{}".format(m_ref)) if m_ref != "" else self.file.write("")
        self.file.write(
            "\n\t\t\tposition\t{}\t{}\t{}".format(viapoint_pos[0], viapoint_pos[1], viapoint_pos[2])
        ) if viapoint_pos else self.file.write("")
        self.file.write("\n\t\tendviapoint\n")
        return 0
