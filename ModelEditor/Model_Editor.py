...
class MainWindow(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        self.modify_button = tk.Button(
            self.frame_converted, text="Modify model", width=10, command=self.modify, state=tk.DISABLED
        )
        ...
        self.analyse_button = tk.Button(self.frame_original, text="Read file", command=self.analyse, state=tk.DISABLED)
    ...
    def check_path(self):
        ...
        if os.path.exists(self.original_path):
            if self.original_path.find(".osim") > -1:
                ...
                self.check_status = (
                    "file found but extension is not recognized.\n" "Please indicate the type of file : "
                )
                self.file_exist = True
    ...
    def analyse(self):
        ...
        for segment in self.model.get_segments():
            index_segment = self.tree.insert(
                index_segments, "end", segment.get_name(), text="segment " + segment.get_name()
            )
            index_segment_parameters = self.tree.insert(
                index_segment, "end", segment.get_name() + "_parameters", text="Parameters"
            )
            self.tree.insert(
                index_segment_parameters,
                "end",
                segment.get_name() + "parent",
                text="parent",
                values=("", segment.get_parent()),
            )
            self.tree.insert(
                index_segment_parameters,
                "end",
                segment.get_name() + "_mass",
                text="mass",
                values=("", segment.get_mass()),
            )
            self.tree.insert(
                index_segment_parameters,
                "end",
                segment.get_name() + "_inertia",
                text="inertia",
                values=("", segment.get_inertia()),
            )
            self.tree.insert(
                index_segment_parameters,
                "end",
                segment.get_name() + "_com",
                text="center of mass",
                values=("", segment.get_com()),
            )
            self.tree.insert(
                index_segment_parameters,
                "end",
                segment.get_name() + "_rt_in_matrix",
                text="rt_in_matrix",
                values=("", segment.get_rt_in_matrix()),
            )
            self.tree.insert(
                index_segment_parameters,
                "end",
                segment.get_name() + "rot_trans_matrix",
                text="rot_trans_matrix",
                values=("", segment.get_rot_trans_matrix()),
            )
            self.tree.insert(
                index_segment_parameters,
                "end",
                segment.get_name() + "dof_translation",
                text="dof_translation",
                values=("", segment.get_dof_translation()),
            )
            self.tree.insert(
                index_segment_parameters,
                "end",
                segment.get_name() + "dof_rotation",
                text="dof_rotation",
                values=("", segment.get_dof_rotation()),
            )
            markers = segment.get_markers()
            if markers:
                index_markers = self.tree.insert(index_segment, "end", "markers" + segment.get_name(), text="markers")
                for marker in markers:
                    index_marker = self.tree.insert(
                        index_markers, "end", marker.get_name(), text="marker " + marker.get_name()
                    )
                    self.tree.insert(
                        index_marker,
                        "end",
                        marker.get_name() + "_position",
                        text="position in segment",
                        values=("", marker.get_position()),
                    )
                    # TODO add technical and anatomical
        for muscle_group in self.model.get_muscle_groups():
            index_muscle_group = self.tree.insert(
                index_muscle_groups, "end", muscle_group.get_name(), text="muscle group " + muscle_group.get_name()
            )
            muscles = muscle_group.get_muscles()
            if muscles:
                for muscle in muscles:
                    index_muscle = self.tree.insert(
                        index_muscle_group, "end", muscle.get_name(), text="muscle " + muscle.get_name()
                    )
                    index_muscle_parameters = self.tree.insert(
                        index_muscle, "end", muscle.get_name() + "_parameters", text="Parameters"
                    )
                    self.tree.insert(
                        index_muscle_parameters,
                        "end",
                        muscle.get_name() + "type",
                        text="type",
                        values=("", muscle.get_type()),
                    )
                    self.tree.insert(
                        index_muscle_parameters,
                        "end",
                        muscle.get_name() + "state_type",
                        text="state_type",
                        values=("", muscle.get_state_type()),
                    )
                    self.tree.insert(
                        index_muscle_parameters,
                        "end",
                        muscle.get_name() + "optimal_length",
                        text="optimal_length",
                        values=("", muscle.get_optimal_length()),
                    )
                    self.tree.insert(
                        index_muscle_parameters,
                        "end",
                        muscle.get_name() + "maximal_force",
                        text="maximal_force",
                        values=("", muscle.get_maximal_force()),
                    )
                    self.tree.insert(
                        index_muscle_parameters,
                        "end",
                        muscle.get_name() + "tendon_slack_length",
                        text="tendon_slack_length",
                        values=("", muscle.get_tendon_slack_length()),
                    )
                    self.tree.insert(
                        index_muscle_parameters,
                        "end",
                        muscle.get_name() + "pennation_angle",
                        text="pennation_angle",
                        values=("", muscle.get_pennation_angle()),
                    )
                    self.tree.insert(
                        index_muscle_parameters,
                        "end",
                        muscle.get_name() + "max_velocity",
                        text="max_velocity",
                        values=("", muscle.get_max_velocity()),
                    )
                    pathpoints = muscle.get_pathpoints()
                    if pathpoints:
                        index_pathpoints = self.tree.insert(
                            index_muscle, "end", "pathpoints" + muscle.get_name(), text="pathpoints"
                        )
                        for pathpoint in pathpoints:
                            index_pathpoint = self.tree.insert(
                                index_pathpoints, "end", pathpoint.get_name(), text="pathpoint " + pathpoint.get_name()
                            )
                            self.tree.insert(
                                index_pathpoint,
                                "end",
                                pathpoint.get_name() + "_parent",
                                text="parent",
                                values=("", pathpoint.get_parent()),
                            )
                            self.tree.insert(
                                index_pathpoint,
                                "end",
                                pathpoint.get_name() + "_position",
                                text="position in parent muscle",
                                values=("", pathpoint.get_position()),
                            )
    ...
    def modify(self):
        ...
        self.list_file_action = tk.OptionMenu(
            self.label_modify, self.var_action, *self.options_action, command=self.show_item_type
        )
        ...
        self.list_file_item = tk.OptionMenu(
            self.label_modify, self.var_item, *self.options_item, command=self.show_item_names
        )
    ...
    def show_item_names(self, valu):
        ...
        self.list_file_names = tk.OptionMenu(
            self.label_modify, self.var_names, *self.options_names, command=self.show_item_parameters
        )
