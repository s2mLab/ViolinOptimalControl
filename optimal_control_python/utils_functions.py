...
def prepare_generic_ocp(biorbd_model_path, number_shooting_points, final_time, x_init, u_init, x0, acados, use_sx):
    ...
    objective_functions.add(
        Objective.Lagrange.MINIMIZE_TORQUE, node=Node.ALL, index=bow.hair_idx, weight=weight2, list_index=2
...
def warm_start_nmpc(sol, ocp, window_len, n_q, n_qdot, n_tau, biorbd_model, acados, shift=1):
    ...
    if not acados:
        ...
        lam_g[: ((n_q + n_qdot) * shift) * (window_len - 1)] = sol["lam_g"][
            (shift * (n_q + n_qdot)) : (n_qdot + n_q) * shift * window_len
        ]
        # shift n_q + n_qdot * shift var
        lam_g[(n_q + n_qdot) * shift * (window_len - 1) : (n_q + n_qdot) * shift * window_len] = sol["lam_g"][
            (n_q + n_qdot) * shift * (window_len - 1) : (n_q + n_qdot) * shift * window_len
        ]
        # last 20 var are copied
        lam_g[(n_q + n_qdot) * shift * window_len : -3 * shift] = sol["lam_g"][
            ((n_q + n_qdot) * window_len + 3) * shift :
        ]
